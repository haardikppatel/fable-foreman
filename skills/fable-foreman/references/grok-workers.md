...
The verified account on this machine authenticates via an OAuth/OIDC session against grok.com, not an API key — a subscription, the same shape as a ChatGPT-subscription Codex login, not metered per call against a card. **Grok has no opt-in pre-approval flag** — SKILL.md's Codex opt-in (Step 0 item 4) is Codex-only. Treat Grok spend under the ordinary rule: journal it, and step a Grok dispatch down or stop under budget pressure exactly as for any other seat.
